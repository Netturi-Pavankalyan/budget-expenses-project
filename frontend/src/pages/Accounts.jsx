...
export default function Accounts({ isDark, toggleTheme }) {
  ...
  const openAddModal = () => {
    setEditingId(null);
    setName(''); setBank(''); setType(ACCOUNT_TYPES[0]); setBalance('');
    setErrorMsg('');
    setIsOpen(true);
  };
  ...
  const openEditModal = (acc) => {
    setEditingId(acc.id);
    setName(acc.name); setBank(acc.bank); setType(acc.type); setBalance(String(acc.balance));
    setErrorMsg('');
    setIsOpen(true);
  };
    ...
    <div className={`flex min-h-screen transition-colors ${isDark ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={openAddModal} className="flex items-center space-x-2 bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors">
            <Plus size={18} />
        ...
        <div className="mb-10">
          ...
          {!hasLoadedOnce && fetching ? (
            ...
            <div className={`text-center py-12 rounded-xl border ${isDark ? 'text-gray-500 bg-[#12121a] border-gray-800' : 'text-gray-400 bg-white border-gray-200'}`}>
                  ...
                  <div className="flex items-center space-x-4 w-full md:w-auto justify-between md:justify-end">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {confirmingDeleteId === acc.id ? (
                        ...
                        <button onClick={() => setConfirmingDeleteId(acc.id)} className="p-2 rounded-lg text-gray-400 hover:text-red-400 hover:bg-gray-800 transition-colors" title="Delete">
                          <Trash2 size={16} />
                        </button>
                      )}
                    </div>
        ...
        {isOpen && (
          <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
            <div className={`w-full max-w-md rounded-xl border p-6 ${isDark ? 'bg-[#12121a] border-gray-800' : 'bg-white border-gray-200'}`}>
              <div className="flex justify-between items-center mb-6">
