    ...
    <div className={`flex min-h-screen transition-colors ${isDark ? 'bg-[#0a0a0f] text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => { setCurrentDate(new Date()); setSelectedDay(new Date().getDate()); }} className={`px-4 py-2 border rounded-lg text-sm transition-colors ${isDark ? 'bg-[#12121a] border-gray-800 text-gray-400 hover:text-white' : 'bg-white border-gray-300 text-gray-600 hover:text-black'}`}>Today</button>
        </div>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div
                    ...
                    className={`h-16 rounded-lg flex flex-col items-center justify-start pt-2 text-sm relative transition-colors cursor-pointer ${
                    ...
                    {hasEvents && (
                      <div className="flex space-x-1 mt-1">
                        {hasEvents.slice(0, 3).map((ev, idx) => (
                          <div key={idx} className={`w-1.5 h-1.5 rounded-full ${isSelected ? 'bg-white' : 'bg-blue-500'}`}></div>
                        ))}
                      </div>
                    )}
                  </div>
        ...
        {selectedDay && (
          <div className={`mt-6 p-6 rounded-xl border transition-colors ${isDark ? 'bg-[#12121a] border-gray-800' : 'bg-white border-gray-200 shadow-sm'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-semibold">{selectedDateLabel}</h3>
              {selectedDayEvents.length > 0 && (
                <span className="text-sm font-semibold text-red-400">Total: -${selectedDayTotal.toFixed(2)}</span>
              )}
            </div>

            {selectedDayEvents.length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-6">No expenses on this day.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {relevantBudgets.length === 0 ? (
                    <p className="text-sm text-gray-500">No budget set for these categories.</p>
                  ) : (
                    <div className="space-y-3">
                      {relevantBudgets.map((b) => {
                        const isOver = b.remaining_amount < 0;
                        return (
                          <div key={b.id} className={`p-3 rounded-lg ${isDark ? 'bg-[#1e1e2e]' : 'bg-gray-50'}`}>
